  ...
  <div class="product-card">
    <router-link :to="`/product/${product._id}`" class="product-link">
      ...
      <div class="product-info">
        ...
        <div class="product-rating" v-if="product.rating">
          <div class="stars">
            <span v-for="star in 5" :key="star" :class="star <= Math.round(product.rating.average) ? 'star-filled' : 'star-empty'">
              ★
            </span>
          </div>
          <span class="rating-text">({{ product.rating.total_reviews }})</span>
        </div>-->

        <div class="product-attributes" v-if="product.attributes && product.attributes.length > 0">
          <span 
          ...
          <span v-if="product.attributes.length > 3" class="attribute-more">
            +{{ product.attributes.length - 3 }}
          </span>
        </div>

        <div class="product-price">
          <span v-if="hasDiscount" class="old-price">{{ formatPrice(product.original_price) }}₫</span>
    ...
    >
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6 16C6 17.1046 5.10457 18 4 18C2.89543 18 2 17.1046 2 16C2 14.8954 2.89543 14 4 14C5.10457 14 6 14.8954 6 16Z" fill="currentColor"/>
        ...
        <path d="M2 2H3.47934C3.92885 2 4.30763 2.32509 4.37781 2.76894L6.62219 16.2311C6.69237 16.6749 7.07115 17 7.52066 17H16C16.5523 17 17 16.5523 17 16V7C17 6.44772 16.5523 6 16 6H5.5M6 10H17" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
